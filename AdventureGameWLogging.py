...
def randomZombieAttack(attackingThing,playerObject):#if a zombie was chosen from random event
    ...
    test_logger.info('Player Attacked by : Zombie',{'Attacked_By': 'Zombie'})
...
def randomHumanAttack(attackingThing,playerObject): #if a human was chosen from random event
    ...
    test_logger.info('Player Attacked by : Human',{'Attacked_By': 'Human'})
    return battle(attackingThing,playerObject) #sends info to battle function.
...
def endingDisplay(playerObject,finNum): #ending text.Displays in
    ...
    extra = {
        'Player' : playerObject.getName(),
        'Weapon' : playerObject.getWeapon(),
        'Food_Found' : int(playerObject.getFood()),
        'Enemies_Killed' : int(playerObject.getKills())
        }
    test_logger.info('Player ending stats', extra=extra)
    test_logger.info(playerObject.getName())
    print()
